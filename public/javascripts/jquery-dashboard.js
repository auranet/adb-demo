...
(function(jQuery) { // Create closure.
  ...
  jQuery.fn.dashboard = function(options) {
    ...
    dashboard.saveColumns = function() {
      ...
      for (var c = 0; c < dashboard.columns.length; c++) {
      ...
      jQuery.extend(params, opts.ajaxCallbacks.saveColumns.data);
      jQuery.post(opts.ajaxCallbacks.saveColumns.url, params, function(response, status) {
    ...
    dashboard.enterFullscreen = function(element) {
      ...
      if (!dashboard.fullscreen) {
        ...
        dashboard.fullscreen = {
          headerElement: jQuery(markup).prependTo(dashboard.element).click(dashboard.exitFullscreen).hide()
        };
      }
    ...
    var throbber = jQuery(opts.throbberMarkup).appendTo(dashboard.element);
    jQuery.getJSON(opts.ajaxCallbacks.getWidgetsByColumn.url, opts.ajaxCallbacks.getWidgetsByColumn.data, init);
    ...
    function init(widgets, status) {
      ...
      for (var c = 0; c < opts.columns; c++) {
        ...
        var col = dashboard.columns[c] = {
          ...
          element: jQuery('<ul id="column-' + c + '" class="column"></ul>').appendTo(dashboard.element)
        };

        // Add the empty placeholder now, hide it and save it.
        col.emptyPlaceholder = jQuery(markup).appendTo(col.element).hide();
        ...
        for (var id in widgets[c]) {
          ...
          col.initialWidgets[id] = dashboard.widgets[id] = widget({
            id: id,
            element: jQuery('<li class="widget"></li>').appendTo(col.element),
            initialColumn: col,
    ...
    function completeInit() {
      ...
      dashboard.sortableElement = jQuery('.column').sortable({
        connectWith: ['.column'],
    ...
    function widget(widget) {
      ...
      widget.minimize = function() {
        jQuery('.widget-content', widget.element).slideUp(opts.animationSpeed);
        var img = jQuery('img', widget.controls.minimize.element);
        img.attr('src', img.attr('src').replace('minimize', 'maximize'));
        widget.minimized = true;
      };
      widget.maximize = function() {
        jQuery('.widget-content', widget.element).slideDown(opts.animationSpeed);
        var img = jQuery('img', widget.controls.minimize.element);
      ...
      widget.saveSettings = function() {
        ...
        jQuery.extend(params, opts.ajaxCallbacks.widgetSettings.data, { id: widget.id });
        jQuery.post(opts.ajaxCallbacks.widgetSettings.url, params, function(response, status) {
          // Merge the response into widget.settings.
          jQuery.extend(widget.settings, response);
      ...
      widget.enterFullscreen = function() {
        ...
        if (!widget.fullscreen.element) {
          ...
          widget.fullscreen = {
            ...
            element: jQuery(markup).appendTo(dashboard.element)
          };
      ...
      widget.addControl = function(id, control) {
        var markup = '<a class="widget-icon ' + id + '-icon"><img src="/images/' + id + '.gif" alt="' + control.description + '" /></a>';
        control.element = jQuery(markup).prependTo(jQuery('.widget-controls', widget.element)).click(control.callback);
      };
      ...
      var throbber = jQuery(opts.throbberMarkup).appendTo(widget.element);
      var params = jQuery.extend({}, opts.ajaxCallbacks.getWidget.data, {id: widget.id});
      jQuery.getJSON(opts.ajaxCallbacks.getWidget.url, params, init);
      ...
      function init(data, status) {
        ...
        jQuery(widgetHTML()).appendTo(widget.element);
        // Save the content element so that external scripts can reload it easily.
        widget.contentElement = jQuery('.widget-content', widget.element);
        jQuery.each(widget.controls, widget.addControl);
      ...
      function initSettings() {
        ...
        var params = jQuery.extend({}, opts.ajaxCallbacks.widgetSettings.data, { id: widget.id });
        jQuery.getJSON(opts.ajaxCallbacks.widgetSettings.url, params, function(response, status) {
          jQuery.extend(widget.settings, response);
          // Build and add the settings form to the DOM.  Bind the form's submit event handler/callback.
          widget.settings.element = jQuery(widgetSettingsHTML()).appendTo(jQuery('.widget-wrapper', widget.element)).submit(widget.saveSettings);
          // Bind the cancel button's event handler too.
          widget.settings.cancelButton = jQuery('.widget-settings-cancel', widget.settings.element).click(cancelEditSettings);
          // Build and add the inner form elements from the HTML markup provided in the AJAX data.
          widget.settings.innerElement = jQuery('.widget-settings-inner', widget.settings.element).append(widget.settings.markup);
          widget.settings.ready = true;
      ...
      function initThrobber() {
        if (!widget.throbber) {
          widget.throbber = jQuery(opts.throbberMarkup).appendTo(jQuery('.widget-wrapper', widget.element));
        }
      };
